...
class processor:
    def __init__(self, file1):
        ...
        self.branch_prediction_details = [] # List to store branch prediction details
        ...
        if self.pipeliningEnabled:
            ...
            if state.isbranch == 0:
                ...
                if btb.find(state.PC) and actual_pc != state.PC_next: # if the branch was in BTB and prediction was wrong then increment the branch misprediction count
                    ...
                    # Record branch prediction details
                    self.branch_prediction_details.append({
                        'pc': state.PC,
                        'instruction': self.riscvCode[state.PC],
                        'prediction': 'Taken' if state.branch_taken else 'Not Taken',
                        'actual': 'Taken' if actual_pc != state.PC + 4 else 'Not Taken',
                        'target': actual_pc,
                        'misprediction': True
                    })
                else:
                    # Record branch prediction details for correct predictions
                    self.branch_prediction_details.append({
                        'pc': state.PC,
                        'instruction': self.riscvCode[state.PC],
                        'prediction': 'Taken' if state.branch_taken else 'Not Taken',
